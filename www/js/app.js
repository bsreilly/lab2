  ...
  $scope.onNext = function (choice) {
    ...
    // Additionally check if player 2 has best 2/3
    {
      ...
      if(window.plugins && window.plugins.nativepagetransitions) {
        ...
        $state.go("player2", {
          player1_choice: processMove().getChoiceAsNumber(choice),
...
lab2.controller('player2', function(processMove, $rootScope, $scope, $ionicModal, $ionicLoading, $ionicViewSwitcher, $state, $stateParams, navigation) {
  ...
  $scope.onNext = function (choice) {
    ...
    if ($scope.processMove.checkWins($stateParams.player1_choice, processMove().getChoiceAsNumber(choice)) == 1) {
      $stateParams.player1_wins += 1;
    }

    /* TODO: Use processMove factory instance to check if player2 has a win */
    if ($scope.processMove.checkWins($stateParams.player1_choice, processMove().getChoiceAsNumber(choice)) == 2) {
      $stateParams.player2_wins += 1;
    } else {
    }
    ...
    else if ($stateParams.player2_wins >= 2) // Check if player2 has best 2/3
    {
      ...
      if(window.plugins && window.plugins.nativepagetransitions) {
        ...
        $state.go("player1", {
          ...
          player2_choice: processMove().getChoiceAsNumber(choice),
          player1_wins: $stateParams.player1_wins,
          player2_wins: $stateParams.player2_wins
        })
      }
